...
def handle_expense(user_id, text):
    # 支出追加
    match_add = re.match(r"支出 (\S+) (\d{1,3}(?:,\d{3})*)円$", text)
    # 支出削除
    match_del = re.match(r"支出 (\S+) (\d{1,3}(?:,\d{3})*)円 削除$", text)

    if match_add:
        category = match_add.group(1)
        amount = int(match_add.group(2).replace(",", ""))
        ...
        amount = int(match_del.group(2).replace(",", ""))
