...
from quiz import start_quiz, answer_quiz, is_in_quiz, is_selecting_level, select_level
...
for event in events:
    ...
    if event["type"] == "message" and event["message"]["type"] == "text":
        ...
        if text in ["日本語クイズ", "クイズ"]:
            result = start_quiz(user_id)
            send_line_reply(reply_token, result)
            return {"status": "ok"}

        if is_selecting_level(user_id, text):
            result = select_level(user_id, text)
            send_line_reply(reply_token, result)
            return {"status": "ok"}

        if is_in_quiz(user_id):
            result = answer_quiz(user_id, text)
            send_line_reply(reply_token, result)
            return {"status": "ok"}
